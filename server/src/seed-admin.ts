...
async function upsertUser(
  ...
  if (existing) {
    ...
    if (wallet) {
      wallet.balances.set("USDT", (mongoose.Types.Decimal128 as any).fromString(userData.usdtBalance));
      wallet.balances.set("BTC", (mongoose.Types.Decimal128 as any).fromString(userData.btcBalance));
      wallet.balances.set("ETH", (mongoose.Types.Decimal128 as any).fromString(userData.ethBalance));
      await wallet.save();
    }
  ...
  await Wallet.create({
    userId: created._id,
    balances: {
      USDT: (mongoose.Types.Decimal128 as any).fromString(userData.usdtBalance),
      BTC: (mongoose.Types.Decimal128 as any).fromString(userData.btcBalance),
      ETH: (mongoose.Types.Decimal128 as any).fromString(userData.ethBalance)
    },
